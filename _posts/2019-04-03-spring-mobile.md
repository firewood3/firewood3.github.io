...
## 핵심
HTTP 요청을 하는 사용자의 디바이스는 데스크탑, 모바일, 테블릿으로 나눌 수 있는데 HTTP 요청 헤더를 보고 사용자의 디바이스 정보를 알 수 있다. 스프링에서는 HTTP 요청을 보고 디바이스를 판별하는 작업을 간편하게 만들어주고, 더 나아가서 디바이스 기기 마다 다른 URL로 리다이렉트 할 수 있는 기능도 제공한다. 
